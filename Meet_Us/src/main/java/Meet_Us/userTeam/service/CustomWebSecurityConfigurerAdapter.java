...
//                                   "/IdSearch",
    ...
    protected void configure(HttpSecurity http) throws Exception {
         ...
         // ROLE_USER, ROLE_ADMIN으로 권한 분리 유알엘 정의 여기 밑에 넣으면 nav가 활성화가 된다. 위에는 그냥 참고만 하자 
	         ...
	         .permitAll()
//	         .antMatchers("/UserSetting").hasAnyRole("ROLE_USER")
	         .antMatchers("/AdminManager").access("hasRole('ROLE_ADMIN')") //hasRole 이걸 붙이고 괄호 해주고 , ROLE_ <-이거 안붙ㅊ이면 오류가 발생한다. 이걸 몇일이나 고민하냐 '안에는 이걸 붙여주세요
